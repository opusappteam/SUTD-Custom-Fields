   ...
   **/
   ...
   function prepareInputForUpdate($input) {
      global $CFG_GLPI;
	    ...
	    // akk Update custom field
			   ...
			   $date=date_create($_POST["datetimepicker"]);
     
			 
           if( isset($_POST["datetimepicker"]) )
		   {

 			 if($_POST["datetimepicker"]!="")
			   {
			   ...
			   {
				  $sql = "Update tbl_customfields SET EmailDate=null  where Ticket_ID=".$this->fields['id'] ; 
		          $DB->query($sql); 
			   }
   ...
   function prepareInputForAdd($input) {
      ...
      global $DB;
          ...
          /* $date=date_create($_POST["datetimepicker"]);
		  $Custom_fields_ticket_id=0;
		  $Custom_fields_ticket_id=$value["ticket_id"]; 
		  
		  
		 //echo($Custom_fields_ticket_id);
		 
           if ($Custom_fields_ticket_id==0)
		   {
			   
			   $Custom_fields_ticket_id=$this->$input['id'];
		 ...
		 if ($_POST["requesttypes_id"]=="2")
		 {
              ...
              VALUES ('".$Custom_fields_ticket_id."', '".date_format($date,"Y/m/d H:i:s")."', '$_POST[PendingReason]')";
			  $DB->query($sql);
	  ...
	  global $CFG_GLPI;
	   ...
	   //akk
             ...
             global $DB;
			  ...
			  {
			  $date=date_create($_POST["datetimepicker"]);
			  $date_full=date_format($date,"Y/m/d H:i:s");
...
//       "<div id='uploadfiles'><input type='file' name='filename[]' value='' size='60'></div>";
   ...
   **/
   ...
   function showForm($ID, $options=array()) {
	 ...
	 //akk hide Pending Reason when its new ticket
    ...
    $defaultpopup='selected';
	  ...
	  ///echo($query_check_exists);
           ...
           // if row is present just retrive from tables 
      ...
      if ($result = $DB->query($query_check_exists)) {
          ...
          while ($data = $DB->fetch_assoc($result))
		  {
			   ...
			   $EmailDate = $data["EmailDate"];
			    ...
			    if($data['PendingR_ID']==1)
                  $Reasonpopup_USER='Selected';
	             if($data['PendingR_ID']==2)
	               $Reasonpopup_VENDOR='Selected';
		  }
	 ...
	 $default_values = self::getDefaultValues();
      ...
      if (!$options['template_preview']) {
         echo "<form method='post' name='form_ticket' enctype='multipart/form-data' action='".
                $CFG_GLPI["root_doc"]."/front/ticket.form.php' onsubmit='return validateForm();'>";
         if (isset($options['projecttasks_id'])) {
           ...
           /*  if ($canupdate) {
			...
			if ($canupdate) {
              /*  echo "&nbsp;<input type='submit' class='submit' name='sla_delete' value='".
					...
					// Modified by AKK on 03 March 2015

					
					echo "&nbsp;<input type='hidden' class='submit' name='sla_delete' value='".
                    $LANG['buttons'][6]."'>";
...
/*             echo $tt->getBeginHiddenFieldValue('due_date');
            ...
            if ($canupdate) {
			   ...
			   //echo "<span id='sla_action' >";
			    ...
			    //Modified by AKK on 03 march 2015 
			   echo "<span id='sla_action' style='display:none'>";
	  ...
	  //MOD by AKK on 03 march 2015
	   ...
	   {
	     echo("<b>Assigned based on URGENCY</b>");
	   }
	 ...
	 /* else { // New Ticket
      ...
      echo "</tr>";
 	  ...
 	  echo "<th $Style> ";
	   //printf(__('%1$s%2$s'), __(''), $tt->getMandatoryMark('urgency'));
      //echo $tt->getEndHiddenFieldText('urgency')."";
	    echo "Pending Reason</th><td>";
      echo "<select id='PendingReason' name ='PendingReason' $Style>
    
               <option  $defaultpopup value='0'>None</option>
    			<option $Reasonpopup_USER   value='1'>USER</option>
			   <option  $Reasonpopup_VENDOR   value='2'>Vendor</option>
 	  ...
 	  echo "<th> ";
	   printf(__('%1$s%2$s'), __('Email Date Time'), $tt->getMandatoryMark('urgency'));
      echo $tt->getEndHiddenFieldText('urgency')."";
	  
	 
	  
	  
	    echo "</th><td><input type='text'  id='datetimepicker' name='datetimepicker'  value='".$EmailDate."'   />   <a href=javascript:NewCal('datetimepicker','ddmmmyyyy',true,24)><img src='../Customfields/css/cal.gif' width='16' height='16' border='0' alt='Pick a date'></a>";
	   echo "</td>";
	   echo "</th> ";
      ...
      if (!$ID
		...
		// echo ("<a target=_blank href='../Customfields/index.php?TicketID=$_GET[id]'  > Priority Reasons </a>");  
